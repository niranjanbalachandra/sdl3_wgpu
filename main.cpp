...
WGPUSurface CreateWGPUSurface(const WGPUInstance& instance, SDL_Window* window)
{
...
#if defined(SDL_PLATFORM_MACOS)
    {
        ...
        result = ImGui_ImplWGPU_CreateWGPUSurfaceHelper(&create_info);
        goto create_done;
    }
...
#else // __EMSCRIPTEN__
    wgpuSetLogCallback(
        [](WGPULogLevel level, WGPUStringView msg, void* userdata) { (void)userdata; fprintf(stderr, "%s: %.*s\n", ImGui_ImplWGPU_GetLogLevelName(level), (int)msg.length, msg.data); }, nullptr
    );
